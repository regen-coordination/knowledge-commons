...
test("OpenAI adapter uses documented Responses settings and no tools", async () => {
  ...
  const savedRequest = providerRequest(capture);
  ...
  // Mutating a caller's capture must not regenerate a different request after persistence.
  capture.posts[0]!.text = "Changed after the request was saved";
  const result = await new OpenAIProvider("test-key", fetcher).extract(
    capture,
    savedRequest,
  );
