...
Use the geo-read skill when the user wants to:
...
Full conceptual details: see `../../../../docs/geo/ontology.md`.
...
`values` and `relations`, like `types`, can aggregate data from every space that contains the entity. Without a filter, the response mixes spaces' properties and relations. Pass the named space's exact ID to both connections:
...
This is the query `geo-write` relies on before any write — paste the discovered IDs to it as `KNOWN IDs` so it doesn't re-discover.
...
**Knowledge Commons types** (18, DAO space `bd727a6ad6ec4a058f681ea9002a1fbf` — source of truth `docs/geo/ontology.md`; also in `COMMONS_TYPE_IDS` in `../../../scripts/geo/src/constants.ts`). Root types (Resource/Agent/Event/Environment) are structural — never applied to entities:

| Type | ID | | Type | ID |
|---|---|---|---|---|
| Resource | `10eb3c89c4254f7b89726f18a6677b36` | | Evidence | `a7bcc070d9e94acdbd5011936d8cb607` |
| Knowledge | `522de6b3580b40e3afd02a5fb2512765` | | Pattern | `af4f13a5b9b44ff18a24780579f291e4` |
| Claim | `96f859efa1ca4b229372c86ad58b694b` | | Protocol | `c38c419810c24cf29dd58f194033fc31` |
| Article | `a2a5ed0cacef46b1835de457956ce915` | | Playbook | `c7e64025536a472c9d220609b064ed78` |
| Topic | `5ef5a5860f274d8e8f6c59ae5b3e89e2` | | Agent | `4bc3786b2d094e42aaa3917b35c8abfe` |
| Funding structure | `658f9a253c244ae6bf0cdcec9c765212` | | Person | `7ed45f2bc48b419e8e4664d5ff680b0d` |
| Environment | `02f8089f2f9f471dbfd06377ce753d65` | | Bioregion | `69a636ec6a5f4d1a8796f2b788d85c9b` |
| Network | `fca084311aa140f28a4d0743c2a59df7` | | Organization | `9547f4fb78744de0a9a9fdd7b4c01c0c` |
| Event | `4d876b81787e41fcab5d075d4da66a3f` | | Tool | `fe1df81dd01a4d24a186425b8713d172` |
...
| Space | ID |
| --- | --- |
| **Knowledge Commons** *(this project's space — DAO, testnet; holds the common ontology)* | `bd727a6ad6ec4a058f681ea9002a1fbf` |
...
If the user names a space **not** in this table, resolve it live — but **confirm the resolved `id` + `page.name` match exactly** before scoping, and STOP on a fuzzy/substituted match. (These same IDs, plus the dataset/exclusion sets, live in `../../../scripts/geo/src/constants.ts` for the actionable skills.)

More type, property, and space IDs live in `../../../scripts/geo/src/constants.ts` and `../../../../docs/geo/ontology.md`.

## Canonical client — use this, not hand-rolled curl

`tooling/.agents/scripts/geo/lib/gql.mjs` is the canonical read client (zero-dependency, Node 18+/Bun, endpoint baked in, 2 retries with backoff on 5xx/429/network + transient GraphQL errors, and it **throws with the full GraphQL error text** — it never swallows errors into an empty `data`, the classic false-"entity not found" bug). Prefer it over ad-hoc `curl`/`fetch` in every script and one-off:

```js
import { query, Q, paginate } from "../../../scripts/geo/lib/gql.mjs"; // adjust relative path
...
17. **Single-space entity view?** Scope both `values` and `relations` by `spaceId`; see [Single-space views](#single-space-views).

## More

- `../../../../docs/geo/ontology.md` — the Knowledge Commons ontology (18 types, buckets, hierarchy) — source of truth for this repo's spaces. Platform mechanics: [GRC-20 spec](https://github.com/geobrowser/grc-20/blob/main/spec.md).
- `../../../scripts/geo/src/constants.ts` — well-known type/property/space IDs (`COMMONS_TYPE_IDS` for the 18 commons types) and the ranked SPACES list.
- `../../../scripts/geo/lib/gql.mjs` + `../../../scripts/geo/lib/gql-cli.mjs` — the canonical client + CLI (see "Canonical client" above).
- `../../../scripts/geo/src/functions.ts` — older reference `gql()` client with retry/backoff and pagination (SDK-coupled; prefer `lib/gql.mjs`).
- Siblings: geo-write skill (writes), geo-curate agent (orchestration), geo-research agent (research).
